...
class EducationDetailsBloc extends AbstractBloc<ContentState, ContentEvent> {
  ...
  Stream<ContentState> get educationDetailsStateStream =>
      this.stateContoller.stream;

  Stream<ContentEvent> get _educationDetailsLoadEventStream =>
      this.eventController.stream.where((event) =>
          event is StartLoadingContentEvent<LoadCurrentEducationsCommand>);

  EducationDetailsBloc(this._queryService) {
    this._educationDetailsLoadEventStream.listen((event) async {
      StartLoadingContentEvent<LoadCurrentEducationsCommand> _event =
          event as StartLoadingContentEvent<LoadCurrentEducationsCommand>;
      ...
      try {
        await this
            ...
            .listen((event) {
          this.updateState(ContentReadyState<List<Education>>(event.payload));
        });
