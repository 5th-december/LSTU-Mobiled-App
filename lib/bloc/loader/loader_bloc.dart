...
class DisciplineTeachersListLoaderBloc extends AbstractPrimitiveLoaderBloc<
  ...
  @override
  Stream<ConsumingState<ListedResponse<TimetableItem>>>
      commandArgumentTranslator(
              loaderFunc, LoadDisciplineTeacherList command) =>
          loaderFunc(
              command.discipline.id, command.education.id, command.semester.id);

  @override
  List<TimetableItem> valueTranslator(ListedResponse<TimetableItem> argument) =>
      argument.payload;
}
...
class DisciplineDetailsLoaderBloc extends AbstractPrimitiveLoaderBloc<
  ...
  @override
  Stream<ConsumingState<Discipline>> commandArgumentTranslator(
...
class ProfilePictureLoaderBloc extends AbstractPrimitiveLoaderBloc<
    LoadProfilePicture, ProfilePicture, ProfilePicture> {
  ProfilePictureLoaderBloc(PersonQueryService queryService) {
    this.loaderFunc = queryService.getPersonProfilePicture;
  }

  @override
  Stream<ConsumingState<ProfilePicture>> commandArgumentTranslator(
      loaderFunc, LoadProfilePicture command) {
    String size = command.size > 150
        ? command.size > 400
            ? 'lg'
            : 'md'
        : 'sm';
...
class PersonalDetailsLoaderBloc
  ...
  @override
  Stream<ConsumingState<Person>> commandArgumentTranslator(
          loaderFunc, LoadPersonDetails command) =>
...
class UserDefinitionLoaderBloc extends AbstractPrimitiveLoaderBloc<
    LoadCurrentUserIdentifier, Person, Person> {
  ...
  @override
  Stream<ConsumingState<Person>> commandArgumentTranslator(
          loaderFunc, LoadCurrentUserIdentifier command) =>
      loaderFunc();
...
class EducationListLoaderBloc extends AbstractPrimitiveLoaderBloc<
  ...
  Stream<ConsumingState<ListedResponse<Education>>> commandArgumentTranslator(
          loaderFunc, LoadUserEducationListCommand command) =>
      loaderFunc(command.person.id);

  @override
  List<Education> valueTranslator(ListedResponse<Education> argument) =>
      argument.payload;
}
...
class SemesterListLoadingBloc extends AbstractPrimitiveLoaderBloc<
  ...
  @override
  Stream<ConsumingState<ListedResponse<Semester>>> commandArgumentTranslator(
          loaderFunc, LoadSemsterListCommand command) =>
      loaderFunc(command.education.id);

  @override
  List<Semester> valueTranslator(ListedResponse<Semester> argument) =>
      argument.payload;
}
...
class SubjectListLoadingBloc extends AbstractPrimitiveLoaderBloc<
  ...
  @override
  Stream<ConsumingState<ListedResponse<Discipline>>> commandArgumentTranslator(
          loaderFunc, LoadSubjectListCommand command) =>
      loaderFunc(command.education.id, command.semester.id);

  @override
  List<Discipline> valueTranslator(ListedResponse<Discipline> argument) =>
      argument.payload;
}

class DisciplineTimetableLoadingBloc extends AbstractPrimitiveLoaderBloc<
    LoadDisciplineTimetable, Timetable, Timetable> {
  DisciplineTimetableLoadingBloc(DisciplineQueryService queryService) {
    loaderFunc = queryService.getDisciplineTimetable;
  }

  @override
  Stream<ConsumingState<Timetable>> commandArgumentTranslator(
          loaderFunc, LoadDisciplineTimetable command) =>
      loaderFunc(
          command.discipline.id, command.education.id, command.semester.id);
...
class DiscussionLoadingBloc extends AbstractPrimitiveLoaderBloc<
    ...
    ListedResponse<DiscussionMessage>,
    ListedResponse<DiscussionMessage>> {
  ...
  ListedResponse<DiscussionMessage> valueTranslator(
          ListedResponse<DiscussionMessage> argument) =>
      argument;
}

class TimetableLoadingBloc extends AbstractPrimitiveLoaderBloc<
    LoadTimetableCommand, Timetable, Timetable> {
  ...
  @override
  Stream<ConsumingState<Timetable>> commandArgumentTranslator(
          loaderFunc, LoadTimetableCommand command) =>
      loaderFunc(
          command.education.id, command.semester.id, command.weekType.type);

  @override
  Timetable valueTranslator(Timetable argument) => argument;
}

class ExamsTimetableLoadingBloc extends AbstractPrimitiveLoaderBloc<
    LoadExamsTimetableCommand, ListedResponse<Exam>, List<Exam>> {
  ExamsTimetableLoadingBloc(EducationQueryService educationQueryService) {
    loaderFunc = educationQueryService.getExamsTimetable;
  }

  @override
  Stream<ConsumingState<ListedResponse<Exam>>> commandArgumentTranslator(
          loaderFunc, LoadExamsTimetableCommand command) =>
      loaderFunc(command.education.id, command.semester.id);
...
class PublicationsListLoaderBloc extends AbstractPrimitiveLoaderBloc<
    LoadPublicationsListCommand,
    ListedResponse<Publication>,
    List<Publication>> {
  PublicationsListLoaderBloc(AchievementQueryService achievementQueryService) {
    loaderFunc = achievementQueryService.getPublicationsList;
  }

  @override
  Stream<ConsumingState<ListedResponse<Publication>>> commandArgumentTranslator(
          loaderFunc, LoadPublicationsListCommand command) =>
      loaderFunc(command.person.id);

  @override
  List<Publication> valueTranslator(ListedResponse<Publication> argument) =>
      argument.payload;
}

class AchievementsListLoaderBloc extends AbstractPrimitiveLoaderBloc<
    LoadAchievementsListCommand,
    ListedResponse<Achievement>,
    List<Achievement>> {
  AchievementsListLoaderBloc(AchievementQueryService achievementQueryService) {
    loaderFunc = achievementQueryService.getAchievementList;
  }

  @override
  Stream<ConsumingState<ListedResponse<Achievement>>> commandArgumentTranslator(
          loaderFunc, LoadAchievementsListCommand command) =>
      loaderFunc(command.person.id);

  @override
  List<Achievement> valueTranslator(ListedResponse<Achievement> argument) =>
      argument.payload;
}

class AchievementsSummaryLoaderBloc extends AbstractPrimitiveLoaderBloc<
    LoadAchievementsSummaryCommand, AchievementsSummary, AchievementsSummary> {
  AchievementsSummaryLoaderBloc(
      AchievementQueryService achievementQueryService) {
    loaderFunc = achievementQueryService.getAchievementsSummary;
  }

  @override
  Stream<ConsumingState<AchievementsSummary>> commandArgumentTranslator(
          loaderFunc, LoadAchievementsSummaryCommand command) =>
      loaderFunc(command.person.id);

  @override
  AchievementsSummary valueTranslator(AchievementsSummary argument) => argument;
}

class TasksListLoaderBloc extends AbstractPrimitiveLoaderBloc<
    LoadTasksListCommand, ListedResponse<StudentWork>, List<StudentWork>> {
  TasksListLoaderBloc(DisciplineQueryService disciplineQueryService) {
    loaderFunc = disciplineQueryService.getStudentWorkList;
  }

  @override
  Stream<ConsumingState<ListedResponse<StudentWork>>> commandArgumentTranslator(
          loaderFunc, LoadTasksListCommand command) =>
      loaderFunc(
          command.education.id, command.semester.id, command.discipline.id);

  @override
  List<StudentWork> valueTranslator(ListedResponse<StudentWork> argument) =>
      argument.payload;
}
