...
class PersonalDataFormBloc
  ...
  Stream<ProducingEvent> get _personalDataUpdateEventStream =>
      this.eventController.stream.where((event) =>
          event is ProduceResourceEvent<PersonalData, UpdateProfileInformation>);
  ...
  PersonalDataFormBloc(PersonQueryService queryService) {
    this._personalDataInitEventStram.listen((event) {
      ...
      this.updateState(ProducingInitState<PersonalData>(initData: _event.resourse));
    });

    this._personalDataUpdateEventStream.listen((event) async {
      var _event = event as ProduceResourceEvent<PersonalData, UpdateProfileInformation>;
      PersonalData profileEditionData = _event.resourse;

      this.updateState(ProducingLoadingState<PersonalData>(data: profileEditionData));
      ...
      try {
        ...
        this.updateState(ProducingReadyState<PersonalData, bool>(data: profileEditionData, response: updated));
      } on ValidationException catch(e) {
        this.updateState(ProducingInvalidState<PersonalData>(null, data: profileEditionData)); // TODO: Add server validation errors
        return;
      } on Exception catch (e) {
        this.updateState(ProducingErrorState<PersonalData>(e, data: profileEditionData));
      }
