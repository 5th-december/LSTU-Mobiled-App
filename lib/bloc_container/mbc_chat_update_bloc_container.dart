...
class MbCChatUpdateBlocContainer {
  AmqpService _amqpService;
  AmqpConfig _amqpConfig;

  Map<String, MbCChatUpdateConsumerBloc> _storage;

  MbCChatUpdateBlocContainer._(
      Map<String, MbCChatUpdateConsumerBloc> watchingBlocs,
  ...
  static Future<MbCChatUpdateBlocContainer> init(UtilQueryService queryService,
    ...
    final storageData = <String, MbCChatUpdateConsumerBloc>{};
    for (Dialog dialogIdentifier in dialogIndetifiersList) {
      MbCChatUpdateConsumerBloc bloc = await _initBloc(
          dialogIdentifier, personIdentifier, amqpService, amqpConfig);
      storageData[dialogIdentifier.id] = bloc;
    }
    return MbCChatUpdateBlocContainer._(storageData, amqpConfig, amqpService);
  }

  Future<MbCChatUpdateConsumerBloc> getBloc(
      Dialog dialog, Person person) async {
    if (this._storage.containsKey(dialog.id)) {
      return this._storage[dialog.id];
    }

    MbCChatUpdateConsumerBloc createdBloc =
        await _initBloc(dialog, person, this._amqpService, this._amqpConfig);
    this._storage[dialog.id] = createdBloc;
    return createdBloc;
  }

  static Future<MbCChatUpdateConsumerBloc> _initBloc(Dialog dialog,
      Person person, AmqpService amqpService, AmqpConfig amqpConfig) async {
    final bloc = MbCChatUpdateConsumerBloc(
