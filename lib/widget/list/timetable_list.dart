...
import 'package:lk_client/widget/chunk/timetable_list_item.dart';
...
class _TimetableListState extends State<TimetableList>
  ...
  Widget build(BuildContext context) {
    this._timetableLoadingBloc.eventController.add(
        StartConsumeEvent<LoadTimetableCommand>(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.hasData) {
          ...
          if (state is ConsumingReadyState<Timetable>) {
            ...
            final timetableWeek = loadedTimetable.weeks.firstWhere(
                (TimetableWeek week) => week.type == widget.week.type,
                orElse: () => null);

            if (timetableWeek == null) {
              return Center(
                  child: Text('Запрошенная учебная неделя не найдена'));
            }
                  ...
                  return Column(
                    ...
                    children: () {
                      ...
                      for (int i = 0; i != lessons.length; ++i) {
                        dayLessonWidgets.add(Container(
                          child: TimetableListItem(
                            lesson: lessons[i],
                          ),
                        ));
                        if (i != lessons.length - 1) {
                          dayLessonWidgets.add(SizedBox(height: 4.0));
                        }
                      }
