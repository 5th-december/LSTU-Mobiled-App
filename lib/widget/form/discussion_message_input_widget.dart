...
class DiscussionMessageInputWidget extends StatefulWidget {
  ...
  @override
  _DiscussionMessageInputWidget createState() =>
      _DiscussionMessageInputWidget();
}

class _DiscussionMessageInputWidget
    extends State<DiscussionMessageInputWidget> {
  ...
  void didChangeDependencies() {
    ...
    if (this._bloc == null) {
      ...
      MessengerQueryService messengerQueryService =
          appState.serviceProvider.messengerQueryService;
      AppConfig appConfig = appState.serviceProvider.appConfig;
      FileTransferService fileTransferService =
          appState.serviceProvider.fileTransferService;
      FileLocalManager fileLocalManager =
          appState.serviceProvider.fileLocalManager;
      DiscussionMessageFormBloc discussionMessageFormBloc =
          DiscussionMessageFormBloc(
              messengerQueryService: messengerQueryService);
  ...
  Widget build(BuildContext context) {
    this
        ...
        .eventController
        .sink
        .add(ProducerInitEvent<AttachedFileContent<DiscussionMessage>>());
        ...
        if (snapshot.hasData) {
          ...
          return AttachedMessagingForm(
            ...
            onSendAction: (
                ...
                ExternalLink link}) {
              DiscussionMessage msg = DiscussionMessage(
                  msg: messageText,
                  externalLinks: link != null ? <ExternalLink>[link] : null);

              AttachedFileContent<DiscussionMessage> content =
                  AttachedFileContent<DiscussionMessage>(
                      file: attachment, content: msg);
              /**
               * Событие начала отправки ресурса с прикрепленным контентом
               */
